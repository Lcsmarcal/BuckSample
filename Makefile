...
debug:
	$(BUCK) install //App:ExampleApp --run --simulator-name 'iPhone 11'

debug_release:
	$(BUCK) install //App:ExampleApp --run --simulator-name 'iPhone 11' --config-file ./BuildConfigurations/Release.buckconfig
...
xcode_tests: project
	xcodebuild build test -workspace App/ExampleApp.xcworkspace -scheme ExampleApp -destination 'platform=iOS Simulator,name=iPhone 11,OS=latest' | xcpretty && exit ${PIPESTATUS[0]}
